...
import { FiSettings, FiArrowLeft, FiSun, FiMoon, FiClock, FiActivity, FiZap, FiTrendingUp, FiCalendar } from "react-icons/fi";
...
const getHeatmapStyle = (keyStat, legacyMaximum, isLight) => {
  if (!keyStat) {
    return {
      backgroundColor: isLight ? "rgba(226, 232, 240, 0.6)" : "rgba(255,255,255,0.04)",
      borderColor: isLight ? "rgba(203, 213, 225, 0.8)" : "rgba(255,255,255,0.1)",
  ...
  if (intensity === 0) {
    return {
      backgroundColor: isLight ? "rgba(16, 185, 129, 0.15)" : "rgba(16, 185, 129, 0.18)",
  ...
  if (intensity < 3) {
    return {
      backgroundColor: isLight ? "rgba(245, 158, 11, 0.15)" : "rgba(250, 204, 21, 0.2)",
  ...
  if (intensity < 7) {
    return {
      backgroundColor: isLight ? "rgba(249, 115, 22, 0.18)" : "rgba(249, 115, 22, 0.22)",
      borderColor: "rgba(251, 146, 60, 0.65)",
      color: isLight ? "#c2410c" : "#ffedd5",
    };
  }
  return {
    backgroundColor: isLight ? "rgba(244, 63, 94, 0.18)" : "rgba(244, 63, 94, 0.24)",
...
const Dashboard = () => {
  ...
  const [dashboardTheme, setDashboardTheme] = useState(() =>
    typeof window === "undefined"
      ? "dark"
      : window.localStorage.getItem("growtyping.dashboardTheme") || "dark",
  );
  ...
  if (loading)
    return (
      <div className={`flex items-center justify-center min-h-screen ${isLightTheme ? "bg-slate-50" : "bg-[#0b131e]"}`}>
        <div className="flex flex-col items-center gap-4">
          <div className="w-14 h-14 border-4 border-emerald-500 border-t-transparent rounded-full animate-spin"></div>
          <p className="text-emerald-500 text-base font-semibold tracking-wider animate-pulse">Loading Analytics Dashboard...</p>
        </div>
      </div>
    );
  ...
  return (
    <div
      className={`min-h-screen font-sans transition-colors duration-300 ${
        isLightTheme ? "bg-slate-50 text-slate-900" : "bg-[#0b131e] text-slate-100"
      }`}
    >
      {/* Sticky Header Navbar */}
      <div className={`sticky top-0 z-40 backdrop-blur-xl border-b px-8 py-4 flex items-center justify-between shadow-lg ${
        isLightTheme ? "bg-white/80 border-slate-200" : "bg-[#0f1927]/80 border-slate-800"
      }`}>
        ...
        <div className="flex items-center gap-3">
          {/* Range Selector */}
          <div className={`flex items-center gap-2 border rounded-xl px-3.5 py-1.5 ${
            isLightTheme ? "bg-slate-100 border-slate-300" : "bg-slate-800/80 border-slate-700"
          }`}>
            ...
            >
              <option value="today" className={isLightTheme ? "bg-white text-slate-900" : "bg-[#111c2d] text-white"}>Today</option>
              <option value="lastDay" className={isLightTheme ? "bg-white text-slate-900" : "bg-[#111c2d] text-white"}>Last Day</option>
              <option value="lastWeek" className={isLightTheme ? "bg-white text-slate-900" : "bg-[#111c2d] text-white"}>Last Week</option>
              <option value="lastMonth" className={isLightTheme ? "bg-white text-slate-900" : "bg-[#111c2d] text-white"}>Last Month</option>
              <option value="last6Months" className={isLightTheme ? "bg-white text-slate-900" : "bg-[#111c2d] text-white"}>Last 6 Months</option>
              <option value="thisYear" className={isLightTheme ? "bg-white text-slate-900" : "bg-[#111c2d] text-white"}>This Year</option>
              <option value="previousYears" className={isLightTheme ? "bg-white text-slate-900" : "bg-[#111c2d] text-white"}>Previous Years</option>
            </select>
          </div>
          ...
          >
            {isLightTheme ? <FiMoon className="text-slate-600" size={16} /> : <FiSun className="text-amber-400" size={16} />}
          ...
          <button
            ...
            className={`p-2.5 rounded-xl border transition-all hover:scale-105 ${
              isLightTheme ? "bg-slate-100 border-slate-300 text-slate-700" : "bg-slate-800/80 border-slate-700 text-slate-300"
            }`}
      ...
      <div className="px-8 py-8 max-w-screen-2xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className={`mt-1 text-xs font-medium ${isLightTheme ? "text-slate-500" : "text-slate-400"}`}>
            Showing test results for <span className="text-emerald-400 font-bold">{rangeLabel[range]}</span>
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
          {[
            { label: "Total Sessions", value: stats.totalSessions || 0, icon: <FiActivity className="text-emerald-400" size={24} />, border: "border-emerald-500/20", text: "text-emerald-400" },
            { label: "Total Time", value: `${Math.round(stats.totalTime || 0)} s`, icon: <FiClock className="text-teal-400" size={24} />, border: "border-teal-500/20", text: "text-teal-400" },
            { label: "Average WPM", value: stats.avgWpm !== undefined && stats.avgWpm !== null ? stats.avgWpm.toFixed(1) : 0, icon: <FiZap className="text-amber-400" size={24} />, border: "border-amber-500/20", text: "text-amber-400" },
            { label: "Typing Streak", value: `${streak} days`, icon: <FiTrendingUp className="text-orange-400" size={24} />, border: "border-orange-500/20", text: "text-orange-400" },
          ].map((card, idx) => (
            <div
              key={idx}
            ...
            >
              ...
              <div className={`p-3.5 rounded-xl ${isLightTheme ? "bg-slate-100" : "bg-slate-800/60"}`}>
                {card.icon}
              </div>
            </div>
          ))}
        ...
        <div className="mb-10">
          <div className="flex items-center gap-3 mb-5">
            ...
            <h2 className="text-xl font-bold">Best Record — <span className="text-emerald-400">{rangeLabel[range]}</span></h2>
          </div>
                ...
                >
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">WPM</span>
                      <span className="font-black text-lg text-emerald-400">{record.highestWpm}</span>
                    </div>
                    <div className="h-px bg-slate-200 dark:bg-slate-800"></div>
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Accuracy</span>
                      <span className="font-bold text-teal-400">{record.highestAccuracy}%</span>
                    </div>
                    <div className="h-px bg-slate-200 dark:bg-slate-800"></div>
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Duration</span>
                      <span className="font-bold text-amber-400">{record.longestDuration}s</span>
                    </div>
                  </div>
                </div>
              );
        ...
        <div className="mb-10">
          <div className="flex items-center gap-3 mb-5">
            ...
            <h2 className="text-xl font-bold">All-Time Best <span className="text-amber-400">👑</span></h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {["15s", "30s", "60s", "custom"].map((type) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">WPM</span>
                      <span className="font-black text-lg text-amber-400">{record.highestWpm}</span>
                    </div>
                    <div className="h-px bg-amber-400/20"></div>
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Accuracy</span>
                      <span className="font-bold text-amber-400">{record.highestAccuracy}%</span>
                    </div>
                    <div className="h-px bg-amber-400/20"></div>
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">Duration</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
          ...
          <div className={`p-6 rounded-2xl border transition-all ${
            isLightTheme ? "bg-white border-slate-200" : "bg-[#111c2d] border-slate-800"
          }`}>
            <div className="flex items-center gap-2 mb-5">
              <div className="w-2.5 h-2.5 rounded-full bg-emerald-400"></div>
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Avg WPM by Type</h3>
            </div>
                ...
                return (
                  <>
                    {[0, 1, 2, 3, 4, 5].map((i) => {
                      ...
                      return (
                        <g key={i}>
                          <line x1="35" y1={200 - (val / maxWpm) * 170} x2="580" y2={200 - (val / maxWpm) * 170} stroke={isLightTheme ? "rgba(0,0,0,0.06)" : "rgba(255,255,255,0.05)"} strokeWidth="1" />
                          <text x="0" y={200 - (val / maxWpm) * 170 + 4} fontSize="10" fill="#6B7280">{val}</text>
                        </g>
                      );
                    })}
                    <line x1="35" y1="200" x2="580" y2="200" stroke={isLightTheme ? "rgba(0,0,0,0.12)" : "rgba(255,255,255,0.1)"} strokeWidth="1" />
                  </>
                );
              })()}
              {wpmByType.map((item, index) => {
                ...
                return (
                  <g key={index}>
                    ...
                    <text x={barCenter} y={200 - barHeight - 8} textAnchor="middle" fontSize="11" fill="#34d399" fontWeight="bold">{avgWpm.toFixed(1)}</text>
          ...
          <div className={`p-6 rounded-2xl border transition-all ${
            ...
            <div className="flex items-center gap-2 mb-5">
              ...
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Avg Accuracy by Type</h3>
            </div>
            <svg width="100%" height="220" viewBox="0 0 600 240" preserveAspectRatio="none">
              {[0, 20, 40, 60, 80, 100].map((val, idx) => (
                <g key={idx}>
                  <line x1="35" y1={200 - (val / 100) * 170} x2="580" y2={200 - (val / 100) * 170} stroke={isLightTheme ? "rgba(0,0,0,0.06)" : "rgba(255,255,255,0.05)"} strokeWidth="1" />
              ...
              {accuracyByType.map((item, index) => {
                ...
                return (
                  <g key={index}>
                    ...
                    <text x={barCenter} y={200 - barHeight - 8} textAnchor="middle" fontSize="11" fill="#2dd4bf" fontWeight="bold">{avgAcc.toFixed(1)}</text>
          ...
          <div className={`p-6 rounded-2xl border transition-all ${
            ...
            <div className="flex items-center gap-2 mb-5">
              ...
              <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Top Weak Keys</h3>
            </div>
            <svg width="100%" height="220" viewBox="0 0 600 240" preserveAspectRatio="none">
              {(() => {
                ...
                return (
                  <>
                    {[0, 1, 2, 3, 4, 5].map((i) => {
                      ...
                      return (
                        <g key={i}>
                          <line x1="35" y1={200 - (value / maxMistakes) * 170} x2="580" y2={200 - (value / maxMistakes) * 170} stroke={isLightTheme ? "rgba(0,0,0,0.06)" : "rgba(255,255,255,0.05)"} strokeWidth="1" />
          ...
          <div className={`lg:col-span-3 p-6 rounded-2xl border transition-all ${
            isLightTheme ? "bg-white border-slate-200" : "bg-[#111c2d] border-slate-800"
          }`}>
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-6">
              <div className="flex items-center gap-2">
                <div className="w-2.5 h-2.5 rounded-full bg-amber-400"></div>
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Keyboard Heatmap</h3>
              </div>
              <p className="text-xs text-slate-500">Hover a key to view error rate details</p>
            </div>
            {(() => {
                ...
                <>
                        ...
                        {row.map((key) => {
                          ...
                          return (
                            <div
                              ...
                              style={getHeatmapStyle(keyStat, legacyMaximum, isLightTheme)}
                            >
                              {key}
                            </div>
                          );
                  ...
                  {keyboardHeatmap.length === 0 && (
                    <p className="mt-5 text-center text-xs text-slate-500">
                      Complete a typing test to start building your keyboard heatmap.
                    </p>
                  )}
                </>
              );
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-10">
          {[
            ...
            return (
              <div key={chart.valueKey} className={`p-6 rounded-2xl border transition-all ${
                isLightTheme ? "bg-white border-slate-200" : "bg-[#111c2d] border-slate-800"
              }`}>
                <div className="mb-5 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: chart.color }}></span>
                    <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">{chart.title}</h3>
                  </div>
                  <span className="text-xs text-slate-500">{dailyProgress.length} days</span>
                </div>
                {dailyProgress.length === 0 ? (
                  ...
                  <svg width="100%" height="220" viewBox="0 0 600 230" preserveAspectRatio="none" aria-label={chart.title}>
                    {[0, 0.25, 0.5, 0.75, 1].map((step) => {
                      ...
                      return (
                        <g key={step}>
                          <line x1="42" y1={y} x2="578" y2={y} stroke={isLightTheme ? "rgba(0,0,0,0.06)" : "rgba(148, 163, 184, 0.15)"} strokeWidth="1" />
                    ...
                    {values.map((value, index) => {
                      ...
                      return <circle key={dailyProgress[index]._id.date} cx={x} cy={y} r="4" fill={chart.color} stroke={isLightTheme ? "#ffffff" : "#111827"} strokeWidth="2" />;
                    })}
        ...
        <div className={`rounded-2xl border overflow-hidden transition-all shadow-md ${
          ...
          <div className="px-6 py-5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
            ...
            <span className={`text-xs px-3 py-1 rounded-lg font-semibold border ${
              isLightTheme ? "bg-slate-100 border-slate-200 text-slate-600" : "bg-slate-800/80 border-slate-700 text-slate-400"
            }`}>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead>
                <tr className={isLightTheme ? "bg-slate-50 border-b border-slate-200" : "bg-slate-800/40 border-b border-slate-800"}>
                  {["Date", "Test Type", "WPM", "Accuracy", "Duration", "Chars Typed", "Correct", "Wrong", "Action"].map((th) => (
                    <th key={th} className="px-5 py-3.5 text-left text-xs font-bold text-slate-400 uppercase tracking-widest">
                      {th}
                    </th>
                  ))}
                </tr>
                ...
                {history.length === 0 ? (
                  <tr>
                    <td colSpan="9" className="text-center py-16 text-slate-500">
                      ...
                      <div className="text-sm">No typing history available for this time range</div>
                    </td>
                  </tr>
                ) : (
                  history.map((item, idx) => (
                    <tr key={item._id || idx} className={`transition-colors ${
                      isLightTheme ? "hover:bg-slate-50" : "hover:bg-slate-800/40"
                    }`}>
                      <td className="px-5 py-4 text-xs font-medium text-slate-400">{new Date(item.testDate).toLocaleDateString()}</td>
                      ...
                      <td className="px-5 py-4">
                        <span className="text-sm font-black text-emerald-400">{item.wpm}</span>
                      </td>
                      <td className="px-5 py-4">
                        <span className="text-sm font-bold text-teal-400">{item.accuracy}%</span>
                      </td>
                      <td className="px-5 py-4">
                        <span className="text-sm font-bold text-amber-400">{item.duration}s</span>
                      </td>
                      <td className="px-5 py-4 text-xs text-slate-400 font-medium">{item.charactersTyped}</td>
                      <td className="px-5 py-4">
                      ...
                      <td className="px-5 py-4">
                        <button
                          ...
                          title={item.testText ? "Replay the exact test text" : "Retake this duration"}
                        >
                          Retype
                        </button>
                      </td>
