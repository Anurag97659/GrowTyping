  ...
  return (
    ...
    >
      <div className="w-full flex justify-center mb-10">
        <h1
          className={`text-4xl md:text-5xl font-light tracking-wider leading-[1.1] pb-1 bg-gradient-to-r from-white via-${currentTheme.primary}-200 to-${currentTheme.primary}-400 bg-clip-text text-transparent drop-shadow-[0_8px_25px_rgba(0,0,0,0.45)]`}
        >
          GrowTyping
        </h1>
      </div>
        ...
        <div className={`${currentTheme.card} rounded-lg p-6 w-full max-w-2xl`}>
          <div className="flex flex-wrap items-center justify-between gap-4">
            <button
              ...
              className="font-semibold px-4 py-2 rounded-lg max-w-[160px] truncate text-left shrink-0"
              title={username}
            >
              {username}
            </button>
            ...
            {/* {!loggedIn && (
              <p className="text-xs opacity-60 mt-1 shrink-0">
                Login to save your stats
              </p>
            )} */}

            <div className="flex gap-3 flex-1 justify-center min-w-0">
      ...
      <div className="max-w-7xl w-full px-6 md:px-10 text-3xl leading-relaxed tracking-wide cursor-text h-48 overflow-hidden box-border">
        {(() => {
          ...
          for (let lineIdx = startLine; lineIdx < endLine; lineIdx++) {
            ...
            lines.push(
              <div key={lineIdx} className="whitespace-pre text-left">
      ...
      <div className="mt-10 text-center font-mono">
        <div
          className={`${currentTheme.card} rounded-lg px-12 py-6 hover:border-${currentTheme.primary}-400 transition-colors inline-block`}
        >
          <div className="flex gap-12">
            <div>
              <p
                className={`text-4xl font-bold ${currentTheme.text} drop-shadow-lg`}
              >
                {liveWpm}
              </p>
              <p
                className={`text-${currentTheme.primary}-300 text-sm mt-2 uppercase tracking-wider`}
              >
                WPM
              </p>
            </div>
      ...
      <p
        className={`text-${currentTheme.primary}-500 mt-8 text-sm tracking-wide`}
      >
      ...
      {finishedRef.current && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50">
          <div
            className={`bg-gradient-to-br from-slate-800 to-slate-900 border-2 border-${currentTheme.primary}-500/50 rounded-2xl p-10 max-w-md w-full mx-4 shadow-2xl`}
          >
            <h2
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div
                  className={`bg-slate-700/50 rounded-lg p-4 border border-${currentTheme.primary}-500/30`}
                >
                  <p
                    className={`text-${currentTheme.primary}-300 text-xs uppercase tracking-wider mb-2`}
                  >
                    WPM
                  </p>
                  <p className={`text-3xl font-bold ${currentTheme.text}`}>
                    {testType === "custom"
                      ? Math.round(
                          correctRef.current / 5 / (Math.max(1, elapsed) / 60),
                        )
                      : Math.round(
                          correctRef.current / 5 / (durationMap[testType] / 60),
                        )}
                  </p>
                </div>
                <div
